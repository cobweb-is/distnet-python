...
class ElectricalNetworkGenerator:
    def __init__(self, num_roots, num_levels, num_children_per_node, num_of_circuit_branches):
        self.num_roots = num_roots
        self.num_levels = num_levels
        self.num_children_per_node = num_children_per_node
        self.num_of_circuit_branches = num_of_circuit_branches
        self.node_id = 1
        self.boards = []
    ...
    def generate_board(self, level, position, parent_phase, parent_id=None, parent_circuit_id=None, branch_id=None):
        ...
        if level < self.num_levels:
            # Generate circuits with branches
            for circuit_num in range(self.num_of_circuit_branches):
                circuit_id = f"{parent_circuit_id}-C{circuit_num + 1}" if parent_circuit_id else f"C{self.node_id}-{circuit_num + 1}"
                for i in range(self.num_children_per_node):
                    branch_id = f"{circuit_id}-B{i + 1}"
...
if __name__ == "__main__":
    generator = ElectricalNetworkGenerator(num_roots=2, num_levels=3, num_children_per_node=2, num_of_circuit_branches=2)
