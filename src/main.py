...
# Parameters
C_NUM_ROOTS  = 1
C_NUM_LEVELS = 4 #get_random_int(1, 5)
C_NUM_OF_WAYS = 3 #get_random_int(1, 6)
C_NUM_OF_CCT_BRANCHES = 1# get_random_int(0, 2)
...
class ElectricalNetworkGenerator:
    ...
    # Determine the board phase
        ...
        board = {
            ...
            "board_supply_source_reference_circuit_id": parent_circuit_id,
        ...
        if level < self.C_NUM_LEVELS:
            ...
            for cct_index in range(C_NUM_OF_WAYS):
                ...
                for str_cct_phase in lst_cct_phases: # e.g loop thru the array L1,L2,L3
                    ...
                    if circuit_type == "TP":
                        if str_cct_phase == "L1":
                            self.generate_board(level + 1, cct_index + 1, str_board_phase, board["board_id"], circuit["circuit_id"], circuit["circuit_is_3phase"])
                    else:
                        self.generate_board(level + 1, cct_index + 1, str_board_phase, board["board_id"], circuit["circuit_id"], circuit["circuit_is_3phase"])
                    if level ==1:
                        ...
                        for branch_index in range(C_NUM_OF_CCT_BRANCHES):
                            self.node_id += 1
                            self.generate_board(level + 1, cct_index + 1, str_board_phase, board["board_id"], circuit["circuit_id"],circuit["circuit_is_3phase"], "-B-")
